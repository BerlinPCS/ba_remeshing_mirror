...
template <typename T> using VertexProperty = pmp::VertexProperty<T>;
using Edge = pmp::Edge;
template <typename T> using EdgeProperty = pmp::EdgeProperty<T>;
using Face = pmp::Face;
...
inline const std::string to_string(TerminationReason reason) {
	...
	case TerminationReason::EmptyQueues:
		return "Empty queues";
	case TerminationReason::OperationLimit:
...
template <typename T> class SyncState {
...
struct OpCompare {
	bool operator()(const OpCandidate &a, const OpCandidate &b) const { return a.score < b.score; }
};
