...
TEST(GlobalRemesherTest, HighThresholdExecutesNoEnergyOperations) {
    ...
    EXPECT_EQ(state.termination_reason, TerminationReason::Converged);
}
...
TEST(GlobalRemesherTest, EdgeLossModeDoesNotIncreaseTotalEnergy) {
    ...
    remesher.set_progress_callback([&](bool) {
        observed_losses.push_back(loss::calc_total_edge_loss(input.mesh, settings.target_length));
    });
    ...
    EXPECT_EQ(progress.load().termination_reason, TerminationReason::Converged);
...
TEST(GlobalRemesherTest, HighThresholdCompletesForAllSplitModesAndFixtures) {
    const std::array<std::string, 4> fixtures = {
        "test_split.obj", "test_collapse.obj", "test_flip.obj", "test_smooth.obj"
    };
...
TEST(GlobalRemesherTest, ValenceModeHonorsOperationLimit) {
    SplitMesh input = make_split_mesh();
    RemesherSettings settings;
    settings.target_length = 1.0f;
    settings.op_gain_threshold = 1e-4f;
    settings.flip = FlipMode::VALENCE;
    settings.operation_limit = 1;
    SyncState<ProgressState> progress;
    RemesherPrioGlobal remesher(input.mesh, settings, progress);
    ...
    EXPECT_EQ(state.queue_stats.executed, 1);
    EXPECT_EQ(state.termination_reason, TerminationReason::OperationLimit);
}
