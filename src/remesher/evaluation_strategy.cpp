...
OperationEvaluation EvaluationStrategy::flip(const Mesh& mesh, Edge e) const {
    ...
    const double after_loss = loss::get_edge_loss_from_length(
        pmp::distance(mesh.position(w1), mesh.position(w2)), ctx.target_length);
    ...
    const int before_valence = (v1_v - iv1) * (v1_v - iv1)
                     + (v2_v - iv2) * (v2_v - iv2)
                     + (w1_v - iw1) * (w1_v - iw1)
                     + (w2_v - iw2) * (w2_v - iw2);
    const int after_valence = (v1_v - 1 - iv1) * (v1_v - 1 - iv1)
...
OperationEvaluation EvaluationStrategy::smooth(const Mesh& mesh, Vertex v) const {
    ...
    for (auto neighbor : mesh.vertices(v)) {
        before += loss::get_edge_loss_from_length(
            pmp::distance(mesh.position(v), mesh.position(neighbor)), ctx.target_length);
        after += loss::get_edge_loss_from_length(
            pmp::distance(p_new, mesh.position(neighbor)), ctx.target_length);
    }
