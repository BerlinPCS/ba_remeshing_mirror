...
void RemesherPrioGlobal::enqueue_length_ops(OpQueue &pq, Edge e) {
  ...
  if (enqueue_candidate(pq, OpCandidate(OpType::Split, e)))
    queue_stats.queued++;
  if (enqueue_candidate(pq, OpCandidate(OpType::Collapse, e)))
    queue_stats.queued++;
}

void RemesherPrioGlobal::enqueue_flip(OpQueue &flip_pq, Edge e) {
  flip_versions[e] = current_tick;
  if (enqueue_candidate(flip_pq, OpCandidate(OpType::Flip, e)))
    queue_stats.queued++;
}

void RemesherPrioGlobal::enqueue_smooth(OpQueue &pq, Vertex v) {
  smooth_versions[v] = current_tick;
  if (enqueue_candidate(pq, OpCandidate(OpType::Smooth, v)))
    queue_stats.queued++;
}
...
void RemesherPrioGlobal::enqueue_affected_region(
    OpQueue &pq, OpQueue &flip_pq, const std::vector<Vertex> &vertices) {
  ...
  for (Vertex v : vertices) {
    if (!v.is_valid() || mesh.is_deleted(v))
      continue;
  ...
  current_tick++;
  for (Vertex v : smooth_set)
    enqueue_smooth(pq, v);
  for (Edge e : length_set)
    enqueue_length_ops(pq, e);
  for (Edge e : flip_set)
    enqueue_flip(flip_pq, e);
}
...
bool RemesherPrioGlobal::priority_changed(double old_priority,
                                          double new_priority) const {
...
void RemesherPrioGlobal::publish_queue_state(int queue_size) {
  p_ctx.update([&](ProgressState &state) {
    ...
    state.processed_candidates = processed_candidates;
...
void RemesherPrioGlobal::single_iteration() {
  ...
  const int initial_elements =
      static_cast<int>(mesh.n_edges() + mesh.n_vertices());
  const int operation_limit = r_ctx.operation_limit > 0
                                  ? r_ctx.operation_limit
                                  : std::max(1000, initial_elements * 20);
  const int maximum_queue_size = std::max(1000, initial_elements * 12);
  int energy_ops_since_flip = 0;
  int stale_at_rebuild = 0;
  int popped_at_rebuild = 0;
  bool hit_operation_limit = false;

  auto maybe_rebuild_queues = [&]() {
    const int queue_size = static_cast<int>(pq.size() + flip_pq.size());
    const int recent_pops = queue_stats.popped - popped_at_rebuild;
    const int recent_stale = queue_stats.stale - stale_at_rebuild;
    const bool stale_dominates =
        recent_pops >= 1000 && recent_stale * 2 > recent_pops;
    if (queue_size <= maximum_queue_size && !stale_dominates)
      return;

    pq = OpQueue();
    flip_pq = OpQueue();
    OpQueue &rebuilt_flip_target =
        r_ctx.flip == FlipMode::VALENCE ? flip_pq : pq;
  ...
  while (!pq.empty() || !flip_pq.empty()) {
    ...
    const bool take_flip =
        !flip_pq.empty() &&
        (pq.empty() ||
         energy_ops_since_flip >= std::max(1, r_ctx.flip_frequency));
    ...
    if (r_ctx.flip == FlipMode::VALENCE && cand.type == OpType::Flip) energy_ops_since_flip = 0;
  ...
  p_ctx.update([&](ProgressState &state) {
    if (hit_operation_limit) {
      state.termination_reason = TerminationReason::OperationLimit;
    } else if (r_ctx.flip == FlipMode::EDGE_LOSS) {
      state.termination_reason = TerminationReason::Converged;
    } else {
      state.termination_reason = TerminationReason::EmptyQueues;
    }
  });
}
