...
void AppViewer::reset(const std::string &file_path) {
	...
	log_path << OUT_LOG_DIR << current_file_name << "_" << remesher_names[static_cast<int>(r_type)] << "_"
			 << split_mode_names[static_cast<int>(r_ctx.split)] << "_" << flip_mode_names[static_cast<int>(r_ctx.flip)]
			 << ".csv";
...
void AppViewer::condition_updates() {
	...
	if (ImGui::BeginPopupModal("Remeshing Progress", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		ProgressState state = p_ctx.load();
		const int processed =
			r_type == RemesherType::PRIORITY_GLOBAL ? state.queue_stats.popped : state.metrics.operations;
		const int workload = processed + state.current_queue_size;
		float progress = workload > 0 ? static_cast<float>(processed) / static_cast<float>(workload)
		...
		ImGui::Text("Approximate workload: %d processed + %d queued", processed, state.current_queue_size);
		ImGui::Text("Operations: %d", state.metrics.operations);
		ImGui::Text("Total Edge Loss: %.4f", state.metrics.total_edge_loss);
		if (r_type == RemesherType::PRIORITY_GLOBAL) {
			ImGui::Text("Queue: %d stale, %d rejected, %d rebuilds", state.queue_stats.stale,
						state.queue_stats.rejected, state.queue_stats.rebuilt);
